...
namespace gadget 
{
    ...
    {
        auto const newSize = std::distance(first, last);                                                                            // changed!!!!!!!! onriginal without casting! (size_type)
        auto const oldSize = size();
        auto const isEqualSequence = (size_type) newSize == oldSize && std::equal(first, last, begin());
